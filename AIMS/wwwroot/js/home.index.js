/* ============================================================================
   Home: Card pagination + strict grid mode (authoritative, RO-guarded)
   - 2 rows fixed; columns = 3 → 2 → 1 from GRID width (not viewport)
...
(() => {
    ...
    let lastTotalPages = 0;          // for dots + pager
    let colLockUntil = 0;
    ...
    function render(forceResetPage = false, widthOverride) {
        ...
        if (!pagerDrawnOnce) {
            renderDots(totalPages);
            pagerDrawnOnce = true;
        } else if (totalPages !== lastTotalPages) {
            renderDots(totalPages);
        } else {
            renderDots(totalPages);
        }
    ...
    window.addEventListener('storage', (e) => {
        if (e.key === 'dashboard:visibleTypes') {
            render(true);
        }
    });
    ...
    const ro = new ResizeObserver((entries) => {
        if (grid.classList.contains('hydrating') || !grid.classList.contains('hydrated')) {
            return;
        }
        ...
        if (Math.abs(inlineSize - lastInlineSize) < 1) {
            return;
        }
        lastInlineSize = inlineSize;
        if (performance.now() < colLockUntil) {
            return;
        }
        if (roPending) return;
        roPending = true;

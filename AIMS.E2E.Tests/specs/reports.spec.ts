...
async function selectTestOffice(page) {
    // Wait for modal to fully finish Bootstrap animation
    await page.waitForSelector('#generateOfficeReport.show', { timeout: 5000 });

    const select = page.getByLabel("Office Name *");
    await select.waitFor({ state: "visible" });

    // Reliable way to pick the office
    await select.selectOption({ label: "Test Office" });
}
...
test.describe("Create Office Report", () => {
    test.describe("Success Case", () => {
        test("Name, Office, Start", async ({ reportsPage }) => {
            ...
            await selectTestOffice(reportsPage);
            await reportsPage.getByRole("textbox", { name: "Start Date *" }).fill("2025-11-22");
            await reportsPage.getByRole("button", { name: "Generate Report" }).click();
